...
void CODE_IN_IWRAM matrix_rotate_world(matrix_t m, int alp, int bet, int gam)
{
...
#define AFFINE(I) {u16 c=tb[uf2i(u+=uu)];if(c)fb[I]=c;}
          AFFINE_LOOP;
#undef  AFFINE
#else
            u32* fb = &yeti->viewport.back->pixels[y1][x1];
            int i = x2 - x1, u = u1 - uu;

#define AFFINE(I) {u32 c=tb[uf2i(u+=uu)];if(c)fb[I]=c|(c<<16);}
          AFFINE_LOOP;
#undef  AFFINE
#endif
          }
...
#define AFFINE(I) {u16 c=tb[uf2i(u+=uu)];if(c)fb[I]=RGB_BLEND(c,fb[I]);}
          AFFINE_LOOP;
#undef  AFFINE
#else
            u32* fb = &yeti->viewport.back->pixels[y1][x1];
...
#define AFFINE(I) {u32 c=tb[uf2i(u+=uu)];if(c){c=RGB_BLEND(c,fb[I]&0xFFFF);fb[I]=c|(c<<16);}}
          AFFINE_LOOP;
#undef  AFFINE
#endif
          }
...
#define AFFINE(I) {fb[I] = RGB_BLEND(tb[uf2i(u+=uu)], fb[I]);}
          AFFINE_LOOP;
#undef  AFFINE
#else
            u32* fb = &yeti->viewport.back->pixels[y1][x1];
...
#define AFFINE(I) {u32 c = RGB_BLEND(tb[uf2i(u+=uu)], fb[I]&0xFFFF);fb[I] = c|(c<<16);}
          AFFINE_LOOP;
#undef  AFFINE
#endif
          }
...
#define AFFINE(I) fb[I] = lua[uf2i(l+=ll)][texture[uf2i((v+=vv))&63][uf2i((u+=uu))&63]];
      AFFINE_LOOP;
#undef  AFFINE
      }
      else
      {
        u16* lut = lua[uf2i((lt_l + rt_l) >> 1)];
#define AFFINE(I) fb[I] = lut[texture[uf2i(v+=vv)&63][uf2i(u+=uu)&63]];
      AFFINE_LOOP;
#undef  AFFINE
      }
#else
...
#define AFFINE(I) {u32 c = lua[uf2i(l+=ll)][texture[uf2i((v+=vv))&63][uf2i((u+=uu))&63]];fb[I] = c|(c<<16);}
      AFFINE_LOOP;
#undef  AFFINE
      }
      else
      {
        u16* lut = lua[uf2i((lt_l + rt_l) >> 1)];
#define AFFINE(I) {u32 c = lut[texture[uf2i(v+=vv)&63][uf2i(u+=uu)&63]];fb[I] = c|(c<<16);}
      AFFINE_LOOP;
#undef  AFFINE
      }
#endif
...
{
  s8* model = entity->visual.data;
  yeti_t* yeti = entity->yeti;
  matrix_t m;
  polyclip_t p;
  vec3_t verts[100];
  int i, j, nvertices;
