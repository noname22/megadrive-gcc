...
{
  int lt_i, lt_x, lt_xx, lt_u, lt_uu, lt_v, lt_vv;
  int rt_i, rt_x, rt_xx, rt_u, rt_uu, rt_v, rt_vv;
  int x1, x2, i, j, u, uu, v, vv;
...
#define AFFINE(I) {u32 c = texture[(f2i(v+=vv) << textsize) + f2i(u+=uu)]; fb[I] = c|(c<<16);}
      AFFINE_LOOP;
#undef  AFFINE
#endif
    }
