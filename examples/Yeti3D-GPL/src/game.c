...
{
  e->xx += fixsin(f2i(e->t) - 512) >> 4;
  e->yy += fixcos(f2i(e->t) - 512) >> 4;
}
...
void bullet_collision(entity_t* e1, entity_t* e2)
{
  ...
  if (e2 != yeti->camera)
  {
    e2->life -= 10;
    if (e2->life >= 0)
      start_snd(e2, ENEMY_PAIN);
    else
      start_snd(e2, ENEMY_DETH);
    // bullet explode sound?
    entity_kill(e1);
  }
}
...
void camera_behaviour(entity_t* e)
{
  ...
  if (yeti->keyboard.y && yeti->keyboard.down) e->p -= i2f(20);
  if (yeti->keyboard.y && yeti->keyboard.up) e->p += i2f(20);
  if (yeti->keyboard.left) entity_turn_left(e);
  if (yeti->keyboard.right) entity_turn_right(e);
  if (yeti->keyboard.x) entity_move_left(e);
  if (yeti->keyboard.z) entity_move_right(e);
  if (!yeti->keyboard.y && yeti->keyboard.up) entity_move_forward(e);
  if (!yeti->keyboard.y && yeti->keyboard.down) entity_move_backwards(e);
  entity_default(e, yeti->keyboard.b, yeti->keyboard.c);

  if (yeti->keyboard.a && !yeti->keyboard._a)
  {
    shoot_bullet(e);
    start_snd(e, PLAYER_ATTK);
  }
}
...
void guard_collision(entity_t* e1, entity_t* e2)
{
  ...
  {
    if (!check_snd(e1))
      start_snd(e1, ENEMY_ATTK);
    if (!check_snd(e2))
      start_snd(e2, PLAYER_PAIN);
  }
}
